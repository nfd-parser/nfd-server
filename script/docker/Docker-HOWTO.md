...
依赖：docker compose v2，删除`name: nfd-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name nfd-maven-repo

docker run -it --rm --name nfd-maven \
    -v nfd-maven-repo:/root/.m2 \
...
- api server: http://localhost:10808
